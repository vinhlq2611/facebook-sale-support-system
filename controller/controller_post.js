...
const PostController = {
    async create(req,res){
        try {
            ...
            if (!fb_id || !content || !attachment || !status || !order) {
                return res.json({ data: null, message: "Lack of information" })
            } 
            uploadFile(attachment,'Post '+ fb_id + ' ' + content)
            let result = await PostService.create({ fb_id, content, attachment, status, order })
            return res.json({ data: result, message: "Create post success" })
        } catch (error) {
            logError("Create Post Error", error)
            return res.json({ data: error, message: "Create Post Error" })
        }
    },
